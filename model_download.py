...
def check_model_availability() -> bool:
    ...
    if os.path.exists(model_base_dir + "/model.bin"):
        ...
        if not validate_s3_uri(w_model):
            ...
            logger.info(
                f"Downloading version {w_model} from Huggingface instead"
            )
            return False
        s3 = S3Store(s3_endpoint_url)
        bucket, object_name = parse_s3_uri(w_model)
        success = s3.download_file(
            bucket, object_name, model_base_dir
        )
        if not success:
            logger.error(
                f"Could not download {w_model} into {model_base_dir}"
            )
            return False
        logger.info(
            f"Downloaded {w_model} into {model_base_dir}"
        )
