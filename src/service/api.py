...
from ..main_data_processor import run as transcription_process
...
transcribe_request = api.model(
    "TranscribeRequest",
    {
        'doc_id': 'this_is_not_a_valid_id',
        'location': 'this_is_not_a_valid_S3_URI'
    })
...
class TranscriptionEndpoint(Resource):
    ...
    def post(self):
        ...
        if task is not None:
            return {
                "status": 503,
                "message": "Busy on another transcription"
            }
        # if available: spin up processing
        document_id = params['doc_id']
        processing_id = document_id + str(time)
        task = asyncio.create_task(transcription_process(input_file_path=params['location']))
